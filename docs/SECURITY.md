...
site may call `chia_takeOffer` so player Sage shows an Accept popup. Sage
Accept has no fee box — treasury attaches the XCH fee on `make_offer`
(`TREASURY_PAYOUT_FEE_MOJOS`). Testers can still import the offer by hand if
