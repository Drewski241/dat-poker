...
export function SuperheroFlyWin({ onFinished }: { onFinished?: () => void }) {
  ...
  useEffect(() => {
    const t = window.setTimeout(() => onFinishedRef.current?.(), 4800);
    ...
    >
      <div className="superhero-win-sky" aria-hidden="true">
        {Array.from({ length: 6 }, (_, i) => (
          <span key={i} className="superhero-win-streak" style={{ animationDelay: `${i * 0.12}s` }} />
        ))}
      </div>
  ...
  return (
    <svg className="superhero-win-svg" viewBox="0 0 200 160" aria-hidden="true">
      ...
      <path
        ...
        opacity="0.85"
      />
      <ellipse cx="100" cy="88" rx="28" ry="34" fill="url(#superhero-suit)" />
      <path d="M72 88 C68 108 66 128 70 142 L130 142 C134 128 132 108 128 88 Z" fill="url(#superhero-suit)" />
